...
get_list_stats <- function(matrix_list, weights, verbose = TRUE) {
    ...
    if (
        ...
        stop(
            "All matrices in the list need to have the same dimensions."
        )
    }
    ...
    bigstatsr::big_apply(
        ...
        a.FUN = function(X, ind) {
            ...
            wrap_mean <- function(final_mat, batch) {
                ...
                # calculates the number of samples in which the feature
                # is present, weighted by the dataset weight
                nmat <- (!is.na(batch_mat) + 0) * weights[batch]

                # calculates the weighted standard deviation
                final_mat <- final_mat +
                    (batch_mat -
                        mat_mean_zscore[, ind, drop = FALSE])^2 *
                        weights[batch]
                final_mat[is.na(final_mat)] <- 0
                list(final_mat, nmat)
            }
            ...
            w_m_sum_sq <- Reduce(
                "+",
                lapply(lmats, function(x) x[[2]]^2)
            )

            # creates an unbiased esitmator for a
            # sample based wighted variance calculation
            denominator <- w_m_sum - (w_m_sum_sq / w_m_sum)

            # Divide by zero shouldn't happen because the features have
            # already been filtered properly
            X[, ind] <- sqrt(m_sd / denominator)
            # Already weighted, no need to divide
            NULL
        },
        a.combine = "c",
        block.size = 1000
